...
from .attention_processor import Kandi3AttnProcessorIpAdapter, Kandi3AttnProcessor, LoraKandi3AttnProcessor
...
class UNetKandi3Controlnet(ModelMixin, ConfigMixin):
    ...
    def set_lora_attn_processor(self, rank=128, lora_self_attention=False):
        lora_attn_procs = {}
        lora_attn_procs['feature_pooling.attention.processor'] = Kandi3AttnProcessor()
        lora_attn_procs['down_samples.1.self_attention_block.attention.processor'] = LoraKandi3AttnProcessor(in_channels=384, out_channels=384, context_dim=384, rank=rank) if lora_self_attention else Kandi3AttnProcessor()
        ...
        lora_attn_procs['down_samples.2.self_attention_block.attention.processor'] = LoraKandi3AttnProcessor(in_channels=768, out_channels=768, context_dim=768, rank=rank) if lora_self_attention else Kandi3AttnProcessor()
