...
class AttentionBlock(nn.Module):
    ...
    def forward(
        ...
        if x_base is not None:
            out_base = self.in_norm(x_base, time_embed_base)
            out_base = rearrange(out_base, 'b c h w -> b (h w) c', h=height, w=width)
        else:
            out_base = None
        context_base = set_default_item(exist(context_base), context_base, out_base)
        ...
        if x_base is not None:
            out_base = rearrange(out_base, 'b (h w) c -> b c h w', h=height, w=width)
            x_base = x_base + out_base
            out_base = self.out_norm(x_base, time_embed_base)
            out_base = self.feed_forward(out_base)
            x_base = x_base + out_base
            return x, x_base
...
class DownSampleBlock(nn.Module):
    ...
    def forward(
        ...
        self_attn_out = self.self_attention_block(
            x=x,
            x_base=x_base,
            context=None, 
            context_base=None,
        ...
        if type(self.self_attention_block).__name__ == 'Identity':
            x = self_attn_out
        else:
            x, x_base = self_attn_out
            
        for in_resnet_block, attention, out_resnet_block in self.resnet_attn_blocks:

            x = in_resnet_block(x, time_embed)
            x_base = in_resnet_block(x_base, time_embed_base)
            ...
            if type(attention).__name__ == 'Identity':
                x = attn_out
            else:
                x, x_base = attn_out

            x = out_resnet_block(x, time_embed)
            x_base = out_resnet_block(x_base, time_embed_base)
...
class UNetKandi3Regional(ModelMixin, ConfigMixin):
    @register_to_config
    def __init__(self,
                 model_channels,
                 init_channels=None,
    ...
    def forward(self, x, time, context=None, context_mask=None, image_mask=None, use_projections=False,
                return_dict=True, split_context=False, uncondition_mask_idx=None, control_hidden_states=None, timestep_cond=None,
                ### REGIONAL
                x_base: torch.Tensor = None,
                context_base: torch.Tensor = None,
        ...
        if exist(context_base):
            time_embed_base = self.feature_pooling(time_embed, context_base, context_mask)
        if exist(context):    
            time_embed = self.feature_pooling(time_embed, context)
        ...
        # idx = 0
        # ax[0].imshow(np.array(time_embed[idx].unsqueeze(0).repeat(200, 1).to(torch.float).cpu()))
        # ax[0].axis('off')
        # ax[1].imshow(np.array(time_embed_base[idx].unsqueeze(0).repeat(200, 1).to(torch.float).cpu()))
        # ax[1].axis('off')
        # plt.tight_layout()
        # plt.show()
            
        # print('time_embed.shape:', time_embed.shape)
        # print('time_embed_base.shape:', time_embed_base.shape)
        ...
        for level, down_sample in enumerate(self.down_samples):
            x, x_base = down_sample(
                ...
                image_mask=image_mask,
                base_ratio=base_ratio,
                cross_attention_kwargs=cross_attention_kwargs if level % cross_attention_kwargs['down_sample_blocks_interval'] == 0 else {k: v for k, v in cross_attention_kwargs.items() if k != 'regional_masks'}, # delete regional masks to avoid region control
            )
            if level != self.num_levels - 1:
                hidden_states.append(x)
                hidden_states_base.append(x_base)
        
        for level, up_sample in enumerate(self.up_samples):
            if level != 0:
                if control_hidden_states is not None:
                    control_pop = control_hidden_states.pop()
                    x = torch.cat([x, hidden_states.pop() + control_pop], dim=1)
                    x_base = torch.cat([x_base, hidden_states_base.pop() + control_pop], dim=1)
                else:
                    x = torch.cat([x, hidden_states.pop()], dim=1)
                    x_base = torch.cat([x_base, hidden_states_base.pop()], dim=1)
            x, x_base = up_sample(
                x=x, 
                x_base=x_base, 
                context=context, 
                ...
                image_mask=image_mask,
                base_ratio=base_ratio,
                cross_attention_kwargs=cross_attention_kwargs if level % cross_attention_kwargs['up_sample_blocks_interval'] == 0 else {k: v for k, v in cross_attention_kwargs.items() if k != 'regional_masks'}, # delete regional masks to avoid region control
            )
        x = self.out_layer(x)
        if not return_dict:
            return (x,)
