...
class RegionalKandi3AttnProcessor:
    
    def Kandi3RAttnProcessor_call(
        self,
        attn,
        x,
        context,
        ...
        # print('====> Kandi3RAttnProcessor_call')
        ...
        if exist(context_mask):
            max_neg_value = -torch.finfo(attention_matrix.dtype).max
            if context_mask.shape[0] != attention_matrix.shape[0]: # REGIONAL ATTENTION MASK
                context_mask = rearrange(context_mask, 'i t -> 1 1 i t')
                context_mask = context_mask.repeat(attention_matrix.shape[0], 1, 1, 1)
        ...
        # import numpy as np
        # import matplotlib.pyplot as plt
        # plt.imshow(np.array(attention_matrix[0][0].to(torch.float).cpu()))
        # plt.colorbar()
        # plt.show()
        # #
        out = einsum('b h i j, b h j d -> b h i d', attention_matrix, value)
    ...
    def __call__(
        ...
        # print('====> RegionalKandi3AttnProcessor.__call__')
        # print('base_ratio:', base_ratio)
        # print('x.shape:', x.shape)
        
        if base_ratio is not None:
            # print('HERE (base_ration is not None)')
            out_base = self.Kandi3RAttnProcessor_call(
                attn=attn,
                x=x_base if x_base is not None else x,
        ...
        # consider if this is SELF-ATTENTION or CROSS-ATTENTION
        
        if x.shape == context.shape: # self-attention
            if base_ratio is not None and 'regional_masks' in cross_attention_kwargs:
                regional_prompt_seq_lens = cross_attention_kwargs['regional_prompt_seq_lens']
                hidden_seq_len = x.shape[1] # H * W = 4096 or 1024 or 256 ...
                H, W = int(hidden_seq_len**0.5), int(hidden_seq_len**0.5)
                # initialize self-attended mask
                self_attend_masks = torch.zeros((hidden_seq_len, hidden_seq_len), device=x.device, dtype=torch.bool)
                # initialize union mask
                union_masks = torch.zeros((hidden_seq_len, hidden_seq_len), device=x.device, dtype=torch.bool)
                # regional img attends to corresponding regional img
                # handle each mask
                for i, regional_mask in enumerate(cross_attention_kwargs['regional_masks']):
                    regional_mask = regional_mask.to(x.device)
                    regional_mask = torch.nn.functional.interpolate(regional_mask[None, None, :, :], (H, W), mode='nearest-exact')
                    regional_mask = regional_mask.flatten().unsqueeze(1).repeat(1, regional_prompt_seq_lens[i])
                    # print('regional_mask.shape:', regional_mask.shape)
                    img_size_masks = regional_mask[:, :1].repeat(1, hidden_seq_len)
                ...
                regional_attention_mask = background_and_self_attend_masks
                # import numpy as np
                # import matplotlib.pyplot as plt
                # plt.imshow(np.array(regional_attention_mask.cpu()))
                # plt.show()
            else:
                regional_attention_mask = None
        else: # cross-attention
            if base_ratio is not None and 'regional_masks' in cross_attention_kwargs:
                # each prompt sequence length
                regional_prompt_seq_lens = cross_attention_kwargs['regional_prompt_seq_lens']
                regional_prompt_seq_lens_cumsum = torch.tensor([0, *regional_prompt_seq_lens]).cumsum(dim=0)
                # sum of all regional prompts sequences lengths
                encoder_seq_len = sum(regional_prompt_seq_lens)
                # sequence length of flattened latent
                hidden_seq_len = x.shape[1] # H * W = 4096 or 1024 or 256 ...
                H, W = int(hidden_seq_len**0.5), int(hidden_seq_len**0.5)
                # initialize regional attention mask
                ...
                for i, regional_mask in enumerate(cross_attention_kwargs['regional_masks']):
                    # flatten mask and repeat it for all tokens of regional prompt
                    regional_mask = torch.nn.functional.interpolate(regional_mask[None, None, :, :], (H, W), mode='nearest-exact')
                    regional_mask = regional_mask.flatten().unsqueeze(1).repeat(1, regional_prompt_seq_lens[i])
                    # regional img attends to corresponding txt
                    regional_attention_mask[:, regional_prompt_seq_lens_cumsum[i] : regional_prompt_seq_lens_cumsum[i+1]] = regional_mask
                ...
                regional_attention_mask = context_mask
        
        # print('before Kandi3RAttnProcessor_call')
        
        out = self.Kandi3RAttnProcessor_call(
            attn=attn,
            x=x,
            context=context,
            context_mask=regional_attention_mask,
            image_mask=image_mask
        )
        
        # print('after Kandi3RAttnProcessor_call')
        
        if base_ratio is not None:
            out = out * (1 - base_ratio) + out_base * base_ratio
            # print('out, out_base')
            return out, out_base
