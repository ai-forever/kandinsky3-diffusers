...
class KandinskyV3Pipeline(DiffusionPipeline, LoraLoaderMixin):
    ...
    def prepare_latents(self, batch_size, num_channels_latents, height, width, dtype, device, generator, latents=None):
        shape = (
            ...
            int(height) // self.movq_scale_factor,
            int(width) // self.movq_scale_factor,
        )
        ...
        # print('before init noise sigma:', latents.min(), latents.max())
        latents = latents * self.scheduler.init_noise_sigma
        # print('after init noise sigma:', latents.min(), latents.max())
        return latents
    ...
    def __call__(
        ...
        num_channels_latents = self.unet.config.num_channels
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # print('before scaling:', latent_model_input.min(), latent_model_input.max())
                latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)
                # print('after scaling:', latent_model_input.min(), latent_model_input.max())
