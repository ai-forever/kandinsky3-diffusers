    ...
    def encode_prompt(
        ...
        if do_classifier_free_guidance and negative_prompt_embeds is None:
            ...
            if negative_prompt is not None:
                ...
                negative_prompt_embeds = torch.zeros(prompt_embeds.shape[0], 128, 4096).to(prompt_embeds.dtype).to(prompt_embeds.device)
                negative_attention_mask = torch.zeros(attention_mask.shape[0], 128).to(attention_mask.dtype).to(attention_mask.device)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_pred_uncond = self.unet(
                    ...
                    context_mask=negative_attention_mask,
                    image_mask=None,
                    use_projections=False,
                    split_context=split_context,
                    return_dict=False
