...
import torch
from transformers import CLIPImageProcessor, T5EncoderModel, T5Tokenizer, CLIPVisionModelWithProjection

from ...loaders import LoraLoaderMixin
from ...models import UNetKandi3IpAdapter, VQModel
...
class KandinskyV3PipelineIpAdapter(DiffusionPipeline, LoraLoaderMixin):
    ...
    def get_zero_embed(self, batch_size=1, device=None):
        ...
        print('zero_image_emb', zero_image_emb.shape)
        return zero_image_emb
    
    def prepare_image_emb(self, img):
        cond = (
                self.image_processor(img, return_tensors="pt")
                .pixel_values[0]
                .unsqueeze(0)
    ...
    def encode_prompt(
        ...
        if prompt == "":
            prompt_embeds = torch.zeros_like(negative_prompt_embeds)
            attention_mask = torch.zeros_like(negative_attention_mask)
        return prompt_embeds, negative_prompt_embeds, attention_mask, negative_attention_mask, split_context
        '''
        if prompt == "":
            print('zero')
            return negative_prompt_embeds, negative_prompt_embeds, negative_attention_mask, negative_attention_mask, split_context
        else:
    ...
    def __call__(
        ...
        if do_classifier_free_guidance:
            ...
            if type(img) == Image:
                image_embeddins = self.prepare_image_emb(img)
            else:
                image_embeddins = None
                for i in range(len(img)):
                    if image_embeddins is None:
